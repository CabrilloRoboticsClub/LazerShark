...
Integrates a Targetpoint-TCM with the Ros2 network.
...
class TargetPoint(Node):
    """
    Reads data from a Targetpoint-TCM and publishes each reading as an IMU message to `imu/targetpoint`.
    ...
    def declare_params(self) -> None:
        """
        Declares Ros2 parameters for Targetpoint:
            dev: `/dev/ttyUSB` port to read from.
    ...
    def callback(self) -> None:
        """
        Read data from Targetpoint and publish to `imu/targetpoint`.
        """
        imu_msg = Imu()
        ...
        # Accel is given in g's, so we convert to m/s^2.
